...
func InitialSettings() []model.SettingItem {
	var token string
	var tokenForProxy string
	if flags.Dev {
		token = "dev_token"
		tokenForProxy = "dev_token"
	} else {
		token = random.Token()
		tokenForProxy = random.Token()
	}
...
(?U)access_token=(.*)&`,
		...
		{Key: conf.TokenForProxy, Value: tokenForProxy, Type: conf.TypeString, Group: model.SINGLE, Flag: model.PRIVATE},
